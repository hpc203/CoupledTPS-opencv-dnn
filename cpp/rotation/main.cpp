...
class CoupledTPS_RotationNet
{
public:
	CoupledTPS_RotationNet(string modelpatha, string modelpathb);
...
CoupledTPS_RotationNet::CoupledTPS_RotationNet(string modelpatha, string modelpathb)
{
...
Mat CoupledTPS_RotationNet::detect(const Mat srcimg, const int iter_num)
{
	Mat img;
	...
	this->feature_extractor.setInput(input_tensor);
...
int main()
{
	CoupledTPS_RotationNet mynet("weights/rotation/rotation/feature_extractor.onnx", "weights/rotation/rotation/regressnet.onnx");
    const int iter_num = 3;
	string imgpath = "testimgs/rotation/DRC-D/00150_-8.4.jpg";
	...
	///imwrite("cpp-opencv-dnn-CoupledTPS_Rotation.jpg", combine_img);
	namedWindow("opencv-dnn-CoupledTPS_Rotation", WINDOW_NORMAL);
	imshow("opencv-dnn-CoupledTPS_Rotation", combine_img);
